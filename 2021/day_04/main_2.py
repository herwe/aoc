...
def part_two():
    ...
    lines = [line for x, line in enumerate(lines) if x > 290]  # 290 / # 71
    checker = ''
    for i in lines:
        if i.isdigit():
            checker += i
        ...
        for index, i in enumerate(bingo_numbers):
            for inner_index, j in enumerate(i):
                if current_number == j:
                    ...
                    if check_if_bingo(number_to_be_marked, index):
                        return
...
def check_if_bingo(number_to_be_marked, index):
    ...
    if loop_and_mark(board_count, board_index, lst, number_to_be_marked):
        return True

    board_count = 0
    board_index = 0
    if loop_and_mark(board_count, board_index, bingo_numbers, number_to_be_marked):
        return True
    ...
    for index, i in enumerate(lst):
        ...
        for j in range(5):
            ...
            if tuple(i) == correct_marked:
                if calc_sum_of_winning_board(lst, board_index, number_to_be_marked):
                    return True


def calc_sum_of_winning_board(board, board_index, number_to_be_marked):
    if board_index not in arr:
        arr.append(board_index)

    if len(arr) == len(board)/5:
        ...
        for i in range(my_start, end):
            for j in board[i]:
                if j != 'X':
                    end_sum += j
        final_sum = end_sum * number_to_be_marked
        print(f"Day 4, part two answer is {final_sum} ")
        return True
    else:
        return False
